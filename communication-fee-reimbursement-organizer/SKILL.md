...
Load `references/workflow.md` before doing the work. Load `references/policy-cheatsheet.md` when the user's sources are for 内蒙古准格尔旗力量煤业有限公司 or when the user provides the same buyer information.
...
- Fill phone numbers from the address books, not from invoices, when the address book and invoice disagree. Record the discrepancy in the exception list.
