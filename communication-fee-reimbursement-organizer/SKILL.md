...
description: Organize Chinese corporate phone or communication fee reimbursement packages for 力量煤业. Use when Codex needs to create a 通讯费/电话费报销明细表 from a policy, HR roster, invoice amounts, phone invoices, address book PDFs, and a Word/Excel reimbursement template while strictly preserving template formatting; filling 人员级别 from explicit grade, award title, or ledger岗位 instead of approval wording; applying policy standards only to explicitly covered people/categories and ledger standards to other approved people; sorting people by level high-to-low; recalculating 发票金额 and 实报金额 with over-cap amounts excluded; matching phone numbers; arranging invoice PDFs by department in detail-table order; and checking invoice buyer information.
---
...
- Resolve each person's `报销标准（元/月）` by source priority: use the policy standard only for people, roles, grades, or categories that the policy explicitly covers; for every other person who is reimbursed by approval, strictly use the monthly standard recorded in the phone/fuel reimbursement personnel ledger or newest approved ledger. Do not infer a higher standard from a person's title, generic approval wording, or policy ceiling. If the ledger standard is missing or conflicts with policy, leave the standard pending and record it in the exception/check report.
...
10. Split amounts by person and month according to the invoice period and the template period. Recalculate both `发票金额` and `实报金额` at the person-month level. Resolve the monthly standard first: policy standard for explicitly covered people/categories, ledger standard for other approved people. Reimbursed amount is the lesser of invoice amount and the resolved monthly standard unless the policy says otherwise, and the excess is not reimbursed.
