...
- Sort reimbursement-detail people by resolved personnel level from high to low before assigning `序号` or filling final rows. The sort key is for ordering only; it must not replace the template `级别` display value, which remains the ledger `岗位` text. Use the hierarchy in `references/policy-cheatsheet.md`; preserve source order for people at the same level, and place unresolved levels last with an exception/check note.
