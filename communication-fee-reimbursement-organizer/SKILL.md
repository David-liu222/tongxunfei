...
- Fill the template `级别` column with the person's matched grade or position grade used for the reimbursement standard, such as `正经理级`, `主管级`, `经批准员工`, or another policy-backed grade. Do not write approval-source wording such as `请示审批报销` in the `级别` column; keep approval basis in the audit evidence or exception/check report.
...
4. Build a personnel lookup from the HR roster with name, department, position, grade, employment status, and any notes that affect eligibility. Resolve the template `级别` value from the person's own matched grade/position standard, not from reimbursement approval-source wording.
